...
Every `StructuredInputRef` requires `input_id`, exact `role`, exact `schema_ref`, `object_version`, an absolute `path`, a lowercase 64-character `sha256`, and `media_type=application/json`. Inline payloads, symlinks, non-files, checksum drift, invalid JSON and alternate caller-authored gate rules are rejected. After checksum verification, every key and value at every depth of each structured JSON payload is screened before model validation. The explicit publication-safety contract rejects absolute POSIX, Windows drive-letter and UNC references; home-relative references using `~`, `~user`, HOME, USERPROFILE or HOMEPATH; every embedded `file:` scheme, including opaque and slash forms; credential-bearing URLs; credential-named JSON members with non-empty values; credential assignments using `=` or `:` plus a single token; bearer credentials; and common GitHub/OpenAI/AWS token forms as `unsafe_scientific_reference`. One semantic credential-name rule is reused for structured keys, URL query keys, assignment names and all scientific/source strings, including strings that otherwise satisfy the published-reference shape. After removing case and separator differences, a name is sensitive when it is exactly `auth` or `authorization`; ends in `password`, `secret`, `token`, `credential` or `credentials`; or ends in `key` while its stem begins or ends with an API/private/secret/access/signing/encryption/consumer/client/SSH qualifier. This rule naturally covers names such as `deviceToken`, `webhookSecret`, `databasePassword`, `signingKey`, `apiResponseKey` and `myApiKey` instead of claiming an exhaustive alias list. Exact suffix and boundary semantics keep `tokenization`, `secreted_factor`, `monkey`, `publicKey`, `capillaryKey`, `signalingKey` and `cellStateKey` legal. Rejected content is neither echoed nor published. This is a bounded, tested pattern contract, not a claim to detect every possible secret. Ordinary `bridge://` identifiers, credential-free `http://`/`https://` URLs and scientific prose such as `secret: secreted factor expression` remain legal.
