...
References copied into public profiles—including ProductCase/ProductDefinition, MeasurementSpec, QC, score, validation, snapshot, measurement, evidence, sensitivity and Evidence Family references—must be scheme-shaped or identifier-shaped. Request-local `*_input_ids` are binding keys and deliberately use only the non-empty/unique binding contract.
