...
| `MeasurementSpecV2` | Additive v0.2 measurement-spec context and typed analysis-unit declaration; v0.1 bytes remain unchanged |
...
`ToolRunV2.measurements` intentionally remains bound to the existing `MeasurementResult`; its public v0.2 Schema bytes are unchanged. `MeasurementResultV2` is a separate structured contract for modules that explicitly declare it, and is not silently substituted into the run envelope. It requires paired numerator/denominator and source-run fields, a positive denominator, finite ordered intervals, null values for `missing`/`unavailable`, and a controlled scope for `unknown`. `domain_score` remains null and `score_state` remains unavailable.

The shared P0 lineage spine separates observations, analysis units and independent groups. A capture or graft unit is a technical/within-subject unit and cannot be declared an independence group. Each checksummed assignment artifact is parsed row by row: its observation-set digest, typed hierarchy, analysis units, independence groups and unused declarations must agree with the manifest. P0-01 may generate only `declared` lineage. A `reviewed` or `frozen` label and review reference are trace metadata, not authority; release-sensitive consumers must verify a package-owned typed receipt. `ProductDefinitionCard` therefore remains `draft` in this contract. ProductCase binds an explicit sample or preparation source, exact manifest, checksum, independence scope and complete independence-group set. These contracts record provenance and fail closed; they do not choose mutable biological roles, thresholds, markers or estimands.
