...
| `BiologicalUnitAttestationReceipt` | Caller/data-owner assertion for `analysis_execution`, bound to an immutable declared manifest, assignment, DataView, observation digest, unit contract and external attestation record |
...
| `VisualizationArtifact` | Byte-compatible v0.1 component, data artifact, Evidence IDs and render-file links used by current P0-01/P0-02 `ToolRun` outputs |
| `VisualizationArtifactV2` | Standalone renderer-independent binding for a typed data object, component/version, evidence semantics, provenance, interactions, accessible fallback and hash-bound renders; not yet embedded in `ToolRun` |
| `FigureRegistrySnapshot` | Discoverable component catalog with producer Tool IDs and explicit `legacy_untyped` or `typed_candidate` migration state |
...
The approved [visualization system](BRIDGE_PRD.md#66-visualization-composer-与-web-交互)
defines the user reading order. Current P0-01/P0-02 runs retain
`VisualizationArtifact` v0.1 unchanged. The separately versioned
`VisualizationArtifactV2` and figure registry are available for staged
...
P0-09 consumes a compilation bundle, P0-08 v0.1/v0.2 profiles and versioned Evidence Family, Claim and reconciliation registries. It creates append-only `EvidenceRecord` and `EvidenceRequirement` facts, deterministic reconciliation records, Case or Comparison graph manifests, fixed-column Parquet node/edge tables, a Cytoscape projection, and three typed explanatory figures with complete TSV fallbacks. Missing evidence creates a requirement rather than a zero-valued record; record and family counts are not independent evidence; shadow or exploratory evidence cannot become formal. JSON/Parquet remain authoritative graph facts, while NetworkX is limited to reconstruction, invariant checks and seven named read-only query helpers. LadybugDB is deferred shadow work and is not a release dependency. The complete interface and reason-code contract are documented in the [P0-09 Tool Card](../src/bridge/tool_packages/cards/P0-09.md).
