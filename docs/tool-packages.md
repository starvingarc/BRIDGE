...
| Input → output | Legacy: seven structured evidence objects → `ProliferationStressResponseProfile` v0.2. Method runtime: those objects plus P0-02 V3 lineage, biological-unit mapping, selector/parameter-only method spec, method input and one normalized H5AD → the profile and a `ProcessMethodBundle` bound to the authoritative ProgramSpec checksum. Either mode may accept one compatible `MeasurementSpecV2`; when supplied, every existing program record is projected one-to-one into a checksummed `MeasurementResultV2` without adding thresholds or scores. Both modes publish typed visualization data and deterministic render artifacts. [Tool Card](../src/bridge/tool_packages/cards/P0-06.md). |
...
| Current evidence / status | Outputs remain `candidate/shadow`, `domain_score=null` and do not establish cell fitness, safety or potency. The three typed views preserve declared denominators, method/score units and explicit `not_assessed` states. Numeric reference envelopes, ordered ProtocolIR timelines, numeric LOD/UCB or spike-in recovery curves, and CNV visualization are not produced. |
...
| Current evidence / status | Synthetic H5AD cases exercise single-graft unit binding, file/checksum validation, backed logical-shape rejection before materialization, probability non-repair, raw-count and log-normalized aggregation semantics, all-row composition, external-context binding, reference/program summaries and fail-closed inputs through the public adapter. Results remain `candidate/shadow`, `domain_score=null`; this is engineering validation, not efficacy or release evidence. [Validation](validation/p0_12_expression_analysis_20260827.md). |
