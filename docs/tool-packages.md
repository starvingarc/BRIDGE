...
| Input → output | Compatible six-object aggregation → `OffTargetControlProfile`; nine-object method mode adds P0-02 V3, biological-unit lineage and method inputs → checksummed `OffTargetMethodBundle`. Either mode may add an independent P0-05 MeasurementSpec; projected runs require a checksummed reviewed BiologicalUnitManifest and verify analysis, independence and cell/nucleus units before emitting `MeasurementResultV2`. Both modes publish typed data, exact TSV fallbacks and deterministic SVG/PNG/PDF figures. [Tool Card](../src/bridge/tool_packages/cards/P0-05.md). |
