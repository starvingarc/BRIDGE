...
| Current evidence / status | Fixtures execute every listed first-version parser/check, deterministic reuse, checksum mutation and blocked Markdown/CSV/SVG cases. Leak canaries include paths, host/environment identifiers and credential material; Markdown checks cover GFM-style bare URLs and canonical public DNS. Passed rules are not a privacy proof; `exported` is neither upload nor scientific release. [Report export validation](validation/p0_11_public_safe_export_20260825.md) · [Artifact audit validation](validation/p0_11_public_artifact_audit_20260827.md). |
