...
| Current evidence / status | Synthetic H5AD cases exercise file/checksum validation, raw-count normalization, all-row composition, external-context binding, reference/program summaries and fail-closed inputs through the public adapter. Results remain `candidate/shadow`, `domain_score=null`; this is engineering validation, not efficacy or release evidence. [Validation](validation/p0_12_expression_analysis_20260827.md). |
