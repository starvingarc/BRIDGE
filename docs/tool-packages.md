...
| Current evidence / status | Synthetic end-to-end execution verifies all eight selected methods, target/regional reference separation, biological-unit binding, typed contract refusal, residual gating and one-unit bootstrap degradation. Results remain `candidate/shadow`, `domain_score=null`; engineering execution is not biological validation. [Aggregation validation](validation/p0_03_target_regional.md#record-p0-03-target-regional-20260825) · [Expression validation](validation/p0_03_target_regional.md#record-p0-03-expression-methods-20260826). |
...
| Current evidence / status | `candidate/shadow`, `domain_score=null`. Reference similarity is a forced ranking among supplied labels without calibrated rejection, not biological age or release evidence; ordered sampling points do not establish a trajectory. [Historical validation](validation/p0_04_developmental_compatibility.md#record-p0-04-developmental-compatibility-v0-3). |
...
| Executable implementation | Producer hard-count accounting, role-aware soft composition and eight method selectors: Clopper-Pearson count intervals, hard/soft sensitivity, independence-group bootstrap, rare-state intervals, empirical spike-in detection hit-rate curves, single-state at-least-one binomial planning, source-family disagreement and ordered OOD coordination. |
...
| Input → output | Legacy aggregation → profile v0.2; method runtime adds V3, declared lineage, caller attestation and method inputs → method bundle and typed figures. `hard_count_accounting` uses seven checksummed objects, including V3 and the same ownership bindings, without a mass/method bundle → hard-count profile v0.1. Each mode accepts an optional MeasurementSpec. Count-only projection emits one accounting measurement and three explicit unavailable measurements; it produces no soft-mass figures. The package result schema is a union; old payloads remain v0.2. [Exact fields and examples](../src/bridge/tool_packages/cards/P0-05.md). |
...
| Software | AnnData, Scanpy, decoupler, NumPy, pandas and SciPy, plus the shared Pydantic/JSON Schema runtime; source-bound input reads canonical Parquet through the existing optional PyArrow dependency. UCell, AUCell, pseudobulk and CNV methods remain independent candidates and are not called by this adapter. |
| Input → output | Legacy: seven evidence objects and an optional P0-06 MeasurementSpec → profile v0.3 plus the historical record projection. Method runtime: six core objects (including P0-02 V3 lineage), biological-unit mapping, a caller/data-owner attestation receipt, selector-only method spec/input, a required independent P0-06 MeasurementSpec and the exact selected DataView H5AD → method bundle v0.2, profile v0.3 and one checksummed MeasurementResultV2 per actual method summary. Normalized expression is used directly; integer raw counts receive deterministic in-package 1e4 scaling and `log1p`. Caller-provided precomputed evidence is forbidden in method mode. `method_runtime_source_bound` selects input v0.2 and derives observation states from the exact producer Parquet/manifest pair. Conflicts remain unresolved and stay in whole-product denominators; old v0.1 inputs and result schemas are unchanged. [Tool Card](../src/bridge/tool_packages/cards/P0-06.md). |
