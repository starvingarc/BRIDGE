...
- `bridge.workflow.LocalWorkflowExecutor` provides the single-process state
  ...
  bounded retry and resume without rerunning succeeded steps. It is in-memory and
  does not claim SQLite durability or background-worker behavior.
...
reports, expose HTTP routes or promote any scientific package. Tool execution
remains owned by the registered high-level Tool Package APIs above.
