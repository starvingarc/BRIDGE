...
| 文档权威性 | 本文档是 BRIDGE v2 当前唯一主规范；Registry 和 Task Card 是受其约束的实施附件 |

> **当前实现状态：** 本文中的正式 `domain_score` 描述的是完成独立验证后的目标合同。当前尚无 P0 `ScoreContract` 冻结，因此所有活动模块必须保持 `domain_score=null`；安装成功、候选指标或任务卡完成均不能替代评分验证。
...
本章只确定分析任务、推荐工具组合、基本流程和发布条件。工具版本、参数、环境、许可证、完整输入条件、benchmark 及官方资料统一记录在附录 [Analysis Task Cards](bridge_v2_spec_v0.1/analysis_task_cards.md)。
...
每项分析任务建立独立 [Analysis Task Card](bridge_v2_spec_v0.1/analysis_task_cards.md)，记录：
...
| 项目 | 当前配置 |
| --- | --- |
| 操作系统 | Ubuntu 20.04.1 LTS，Linux 5.15 |
| CPU | 2 × Intel Xeon Gold 6330 |
| CPU 规模 | 56 个物理核心，112 个线程 |
| 内存 | 约 1.0 TiB |
| GPU | 2 × NVIDIA GeForce RTX 3090 |
| GPU 显存 | 每张 24 GiB |
| NVIDIA Driver | 535.183.01 |
| Driver CUDA | 12.2 |
| 主存储 | 37 TB，当前约 11 TB 可用 |
| 次存储 | 73 TB，当前约 9.2 TB 可用 |

存储余量属于动态信息。大规模 reference、空间数据、pySCENIC 或模型任务运行前需要估算输入、临时文件和最终产物大小。
...
1. 优先调用 [Visualization Registry](bridge_v2_spec_v0.1/visualization_registry.md) 中已注册的组件。
...
`ProductCase`、`AnalysisPlan`、`MeasurementSpec`、`ScoreContract`、`MeasurementResult`、`ProductEvidenceObject`、`CaseEvidenceGraph`、`ComparisonEvidenceGraph`、`ComparisonRecord`、`VisualizationArtifact`、`RecommendationCard` 和 `VerifiedReport` 均采用追加式版本管理，详细字段见 [Object Schemas](bridge_v2_spec_v0.1/object_schemas.md)。
...
| 附录 | 文档 | 用途 |
| --- | --- | --- |
| A | [数据与 Reference Registry](bridge_v2_spec_v0.1/data_reference_registry.md) | 数据角色、血缘、状态、访问与评测资格 |
| B | [Tool Registry](bridge_v2_spec_v0.1/tool_registry.md) | 工具、输入、输出、边界、环境和晋升状态 |
| C | [Knowledge Registry](bridge_v2_spec_v0.1/knowledge_registry.md) | 知识快照、许可、evidence family 和允许用途 |
| D | [Conda Environment Contracts](../environments/README.md) | 工具运行所需的通用 Conda 环境合同 |
| E | [Analysis Task Cards](bridge_v2_spec_v0.1/analysis_task_cards.md) | 分析任务合同、验证要求和发布状态 |
| F | [Object Schemas](bridge_v2_spec_v0.1/object_schemas.md) | Agent、证据、比较、可视化和报告对象合同 |
| G | [Visualization Registry](bridge_v2_spec_v0.1/visualization_registry.md) | 正式图表组件、输入绑定和发布要求 |
| H | [v0.1 走查记录](bridge_v2_spec_v0.1/validation_walkthrough_v0.1.md) | 当前桌面走查和待真实团队确认事项 |
