...
| `Q-SPHEREDIFF-v1` | 陈跃军组 3D mDAP；D28 | v1 对象 9,547 | Cell Stem Cell 2025 研究关联比较 | 上游受控 `HRA008865` 与本地对象尚未一一对应；当前只作历史 `analysis_ready` 对照 |
| `Q-MACRODIFF-v1` | 陈跃军组内部 mDA；D14/D21/D28 | v1 对象 57,464；P0-01 原始计数 78,542 | 内部未发表方案与时间序列比较 | 六个 capture 不等于六个独立 preparation；仅限服务器使用 |
| `Q-LEGACY-MSKDA01-v1` | local Studer-protocol D16 | v1 对象 9,046；P0-01 原始计数 11,087 | 已发表 MSK-DA01/Studer 方案背景比较 | Piao et al. 2021 支持方案背景，不证明本地测序文件来自论文数据集 |
