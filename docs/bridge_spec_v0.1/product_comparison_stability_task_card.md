...
> evidence is never converted to zero and all scores remain null.
...
- 当前 `CMP-EFFECT` 在两个独立 sample/preparation 序列上计算 raw delta 与 Hedges g；配对设计、相对变化、区间和方向一致性尚未实现。
...
- 当前 `STAB-CV` 报告 sample/preparation 序列的 CV 与可定义时的 MAD ratio；区间尚未实现。
- 当前 `CMP-JS` 调用 SciPy 计算相同标签 probability-mass 向量的 Jensen-Shannon distance；`CMP-WASS-1D` 仅处理一维有序标量分布，不等同于 graph-Wasserstein。
