...
- 编译器从 exact v0.2 MeasurementResult 解析上述定量事实，并要求其 logical ref、Schema、source checksum 与 P0-08 `measurement_result_bindings` 完全一致；对象目录中对应 MeasurementResult 的 content hash 也必须等于该结构化输入的精确字节 SHA-256，同时核对 ProductCase、domain、MeasurementSpec、EvidenceFamily 和 producer ToolRun state。
