...
| 字段 | 内容 |
|---|---|
| Task ID | `TASK-OFFTARGET-v0.5` |
| 文档版本 | `0.5` |
| 日期 | 2026-09-07 |
| 状态 | `candidate / executable shadow` |
| 上游 | P0-02 V2/V3、预计算 evidence bundle 或 V3 producer composition，以及可选方法运行对象 |
| 输出 | `OffTargetControlResult v0.1` 联合结果；旧模式保持 `OffTargetControlProfile v0.2`，hard-count 模式输出 `OffTargetHardCountProfile v0.1`；可选 checksummed `MeasurementResultV2`；方法模式另含 `OffTargetMethodBundle v0.1` |
...
兼容模式只聚合已经计算好的细胞状态证据。`hard_count_accounting` 模式直接
核算 P0-02 V3 已提供的 whole-selected-view reference-support count，不要求调用方
合成 soft mass 或 evidence bundle；它不把 single-source support 当成 unknown，也不
把 source conflict 解析成产品角色。方法模式进一步执行透明的描述性
区间、independence-group bootstrap、hard/soft sensitivity、输入 spike-in 的
候选检测限、单状态至少一个细胞的二项采样规划以及多来源 OOD 状态协调。三种模式均不读取表达
...
`method_runtime` 恰好包含十个对象；`hard_count_accounting` 恰好包含产品上下文
四对象、P0-02 V3、BiologicalUnitManifest 和 BiologicalUnitAttestationReceipt，
并可选一个 MeasurementSpec。每个
`StructuredInputRef` 均需绝对普通文件路径、Schema URI、对象版本和 SHA-256。

| role | 模式 | Schema / 关键绑定 |
|---|---|---|
| `product_case` | 三者 | ProductDefinition、assay、MeasurementSpec；方法模式另绑定 BiologicalUnitManifest |
| `product_definition_card` | 三者 | supported assay 与 StateRoleMap ref |
| `state_role_map` | 三者 | state-role assignments、证据类别、方向与 provenance |
| `off_target_assessment_spec` | 三者 | map ref/hash、分母、unknown allowlist、rare-state rules |
| `cell_state_evidence_profile` | 三者 | legacy 为 V2；方法与 hard-count 模式为 V3，含 composition 与 DataView lineage |
| `off_target_evidence_bundle` | legacy、方法 | 上游 ref/hash、分母、state/unknown observations、supplied calibration；hard-count 禁止 |
| `biological_unit_manifest` | 方法、hard-count 及任何投影运行 | P0-01 生成的 immutable `declared` analysis-unit 与 independence-group mapping |
| `biological_unit_attestation_receipt` | 方法、hard-count | data owner/caller 对 `analysis_execution` 设计的显式声明；绑定 manifest/assignment/DataView/observation digest、逐项确认、attestor、UTC 时间与外部 attestation ref/hash |
...
| `measurement_spec` | 三者可选 | 独立 P0-05 MeasurementSpecV2；须匹配 assay、产品适用范围、P0-05、三类投影 metric 及 BiologicalUnitManifest 的 analysis/independence/observation units |
...
1. 校验所选模式的六个、十个、七个或八个文件、Schema、版本、媒体类型和 checksum。
2. 校验 ProductCase → ProductDefinitionCard → StateRoleMap、assay、
   MeasurementSpec 与 P0-02 profile 的引用及 checksum 血缘；旧模式另核对 bundle。
...
5. hard-count 模式只接受 complete shadow 的 canonical L1 selected-view composition；
   reconciliation label/state 必须为 `consensus_supported/candidate`、
   `single_source_supported/candidate`、`source_conflict/unresolved`、
   `unknown/unknown`、`ood/ood` 或 `unavailable/unavailable`。四类 role count 加所有
   非 consensus reconciliation count 必须闭合到 selected N；source-specific count
   仅保留追溯，不跨来源相加。
6. 拒绝未映射 state、未声明 unknown reason、未声明的 rare calibration、
   inactive spec 或分母不一致。
7. 按外部 StateRoleMap 将预计算 state observations 或 consensus-supported V3 rows 确定性求和。
8. 旧聚合只有 composition coverage 为 `complete` 时才计算角色 fraction；hard-count fraction 固定为 count/selected N，但不是 soft fraction 或 purity。
9. unknown 只按外部 allowlist 中实际出现的 reason 汇总；hard-count 不把 reconciliation bucket 改写为 biological unknown。
10. 方法模式核对 DataView、BiologicalUnitManifest、unit-level 与 aggregate counts。
11. 执行描述性 exact interval、hard/soft sensitivity 与 independence-group bootstrap。
12. 每个 spike-in 浓度只允许每个 independence group 一次，计算候选检测限与单状态二项规划；按 MethodSpec 固定的上游来源族协调 OOD 状态。
13. 旧模式从 typed visualization data 生成图表；hard-count 模式只发布 JSON 与可选 measurement artifacts，`visualizations=[]`。发布前均再次检查输入 checksum 并原子写入全部 artifact。
...
legacy 与方法模式生成 checksummed `off_target_control_profile.json`；hard-count
模式生成 `off_target_hard_count_profile.json`。三者都将同一对象作为
`ToolRunV2.result` 返回；方法模式原子增加
`off_target_method_bundle.json`。

P0-05 v0.6.0 以 `OffTargetControlResult v0.1` 联合 Schema 返回结果。旧模式继续
返回字节形状不变的 profile v0.2；hard-count 模式返回 profile v0.1，其中完整保留
producer composition，`accounting_basis=producer_reference_support_counts`、
`mass_state=unavailable`、`total_soft_mass=null`，open-set 与 rare detection 均为
`not_assessed`。未提供 MeasurementSpec 时明确记录
`not_requested` 且不生成 normalized measurement；旧模式提供并通过校验时，每条现有
role、unknown 和 rare-state 记录各生成一个 checksummed MeasurementResultV2；
hard-count 按下述固定四项生成。
...
hard-count 的可选 MeasurementSpec 必须恰好声明四项 metric：
`off_target_hard_count_accounting` 为 `inferred`，raw_value 是完整 typed accounting；
`off_target_soft_mass_composition`、`off_target_identity_unknown` 与
`off_target_rare_state_detection` 均为 `unavailable`，所有值与区间字段为 null。
四项均保留 succeeded source-run/provenance，且不得产生 score。无 spec 时不生成
measurement artifact；unavailable 项仍可被 P0-08 读取，不改变 P0-08 语义。
...
- hard-count 为 whole-view reference-support count：不提供 soft mass、per-unit 或 bootstrap estimate；所有角色（包括正数）均为 `cannot_exclude`。
- hard-count 的四个角色都为零只表示没有 consensus-supported role count，不表示生物状态不存在。
...
当前工程验证使用完全合成的结构化对象，覆盖三个模式、hard-count mixed support、
全 single-source、全 conflict、zero-role、闭合与绑定拒绝、四项 measurement 语义、
八个 selector、角色映射
