...
Observed after the 2026-08-14 final review fix set on Python 3.12.13 with the shared development environment's PyArrow 23.0.1 and NetworkX 3.6.1: `168 passed`.

Current source-tree observations:

- the complete source-tree suite passed: `858 passed, 3 warnings`;
...
- Comparison output used a distinct comparison manifest and property-free external EvidenceRecord nodes; stable `external_evidence_bindings` retained the exact source Evidence/Claim/family/P0-08 profile semantics without request-side input IDs, and every accepted Evidence node had a canonical profile dependency edge. Source roles had to form an exact input-ID bijection, and each source manifest's real directory, five authoritative artifacts, raw manifest/record-set checksums, deterministic graph ID, graph/version/ProductCase and complete EvidenceRecord history were verified before use;
...
- manifest opening parsed the checksum-verified JSON/Parquet payload bytes and rejected traversal/absolute filenames, symlinks, checksum/row-count drift, exact node-type-count substitution, forged graph scope, forged graph/set/fact/logical identities, forged node IDs, forged edge IDs/self-hashes, Evidence tier/lifecycle/property drift, semantic edge substitutions or omissions, invalid Requirement satisfaction, disconnected graphs and JSON-fact/Parquet disagreement; base/source preflight inherited the same checks while property-free external nodes remained source-bound rather than being required to expose copied properties;
