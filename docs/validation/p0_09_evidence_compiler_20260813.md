...
Observed after the 2026-08-14 final review fix set on Python 3.12.13 with the shared development environment's PyArrow 23.0.1 and NetworkX 3.6.1: `178 passed`.

Current source-tree observations:

- the complete source-tree suite passed: `868 passed, 3 warnings`;
...
Two independent closure reviews used `git archive` snapshots of fixed head `0ec12655c0ecf83b97e066db9ac3fe16f2b7c6c1` and each reported `Critical=0`, `Important=0`. Both reproduced successful direct-adapter and public-registry runs under legal `key=value` parent directories, reopened the resulting bundles, and confirmed that production Parquet reads use stable bytes. They also rechecked the unified no-cause/no-canary `open` boundary, Comparison full bindings, graph/fact/root identities, checksum-verified-byte handling, Schema/runtime parity and request-local binding-ID semantics without finding a blocker.

The integrator archived exact code commit `ac18dd3`, built wheel `bridge-0.2.0.dev0-py3-none-any.whl` (`sha256:0fd3741b484bc52a80d703ce04d1a746be8fbee4783f899875aa9520d49d27a8`), and installed it into a fresh Python 3.12.13 environment with the declared PyArrow 21.0.0 and NetworkX 3.5 dependencies. The installed-wheel suite passed `868 passed, 3 warnings`. Dependency consistency, 12-tool discovery, P0-09 `implemented` resolution, all 49 installed Schemas, the installed P0-09 Card, packaged knowledge validation and repository policy also passed. This exact wheel includes both the unified public `open` wrapper and the write-after-read Parquet byte-path correction.
...
- the external-binding edge digest was tested only as an internal manifest-to-Parquet consistency binding. It is not a signature or independent authenticity root: a coordinated rewrite of the self-contained manifest, Parquet and checksums is outside query-open detection without the checksummed source Case artifacts. Compilation verified source scientific facts against those source manifests/record sets, and later external provenance traversal stopped at `source_case_graph_required` rather than copying source properties;
...
- manifest opening parsed the checksum-verified JSON/Parquet payload bytes and rejected traversal/absolute filenames, symlinks, checksum/row-count drift, exact node-type-count substitution, forged graph scope, forged graph/set/fact/logical identities, forged node IDs, forged edge IDs/self-hashes, Evidence tier/lifecycle/property drift, semantic edge substitutions or omissions, invalid Requirement satisfaction, disconnected graphs and JSON-fact/Parquet disagreement; adversarial Case manifest, Comparison external-binding manifest, Case JSON fact and Comparison Parquet inputs each produced only exact `manifest_integrity_failed` with `__cause__=None`, and neither rendered traceback nor exception text contained the canary or artifact path; base/source preflight inherited the same wrapper while property-free external nodes remained source-bound rather than being required to expose copied properties;
...
- a legal absolute output path whose parent basename was `batch=one` completed successfully. The adapter's write-after-read roundtrip used stable Parquet bytes, so PyArrow did not inject a Hive partition column; the production query path already used the same byte parser. The only remaining path-based reader is a test-only mutation helper;
