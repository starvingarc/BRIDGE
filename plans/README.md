...
|---|---|---|---|---|
| [P0-01 Optional Mitochondrial QC](p0-01-optional-mitochondrial-qc.md) | Preserve usable count-depth and detected-gene QC when a declared input has no recognizable mitochondrial symbols | Mitochondrial fractions are correctly unavailable, but the current runtime suppresses every candidate QC rule | Review the focused regression and confirm the warning-only boundary | `implementation_complete_review_pending` |
