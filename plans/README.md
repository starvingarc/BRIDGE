...
| [P0-05 Off-target and Unknown Visualization](p0-05-off-target-control-visualization.md) | Show which product observations are assigned outside declared target roles, which remain unresolved, and what rare-state and OOD evidence can or cannot exclude | The package already returns composition, detectability and source-family coordination records but no typed figures | Correct partial-coverage semantics, then render three package-owned evidence figures | `implementation_in_progress` |
