...
|---|---|---|---|---|
| [Agent Runtime Integration](agent-runtime-integration.md) | Execute approved scientific plans through a recoverable, case-scoped local Agent runtime without granting model output scientific authority | The local implementation and optional mitochondrial QC fix pass current-main regression; review is pending | Review the deterministic authority and public-safe model boundary | `implementation_complete_review_pending` |
