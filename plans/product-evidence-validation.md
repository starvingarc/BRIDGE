# Product Evidence Validation Follow-up
...
The installed Web path has exercised explicit intake/choice confirmation, approved QC and cell-state stages, candidate missingness compilation and a blocked internal report. Separately approved installed SDK runs produced descriptive L2 marker/process observations on an explicitly selected QC-filtered view. Preserve those canonical receipts and original inputs; their successful execution is not a product-assessment qualification.

Protocol representation now separates original intent, user supplements, compilation limits and version-bound human review. It is not a batch execution record, proof of all semantic omissions, sample identity or independent replication. The accepted isolated runtime is separate from the normal preview deployment.
...
4. Complete reviewed ProgramSpec/ProtocolIR, product definitions, state roles and required lineage/attestation inputs for genuine P0-03–P0-06 product-assessment modes. Existing descriptive marker/cycle/program results do not supply gate-facing domain MeasurementResults. Retain the specific P0-05/P0-06 module contracts and their separate validation histories. The owner-selected [Seurat v5.5.1 cell-cycle resource](../src/bridge/tool_packages/p0_06_proliferation_stress_response/resources/seurat-cell-cycle-v5.5.1-candidate.json) and [license](../src/bridge/tool_packages/p0_06_proliferation_stress_response/resources/seurat-cell-cycle-LICENSE.txt) are packaged byte-for-byte for explicitly authorized exploratory assembly only; they remain source/version/license-bound candidates, not formal defaults or reviewed science.
...
- Every scientific decision and input has a genuine source, review scope and version; unknown/missing/unavailable states remain distinct.
- A real uploaded case proceeds through explicit confirmation, registration, approved tools, canonical receipts and usable Web results for each newly claimed mode.
- Report values, claims and exported artifacts bind exact qualified evidence and approved policies; unsupported exports remain blocked.
- Domain scores remain null and shadow/unavailable until a ScoreContract is independently frozen and validated.
- Deployment remains a separate action with original sessions, raw data and historical receipts preserved.

No fixture, model output, source-accounting check, compiler success, passing test or PR merge is evidence of biological validity, experimental executability, clinical efficacy/safety, validated potency or GMP release.
