...
| [V1 Output Path Hardening](plans/runtime-output-path-hardening.md) | Preserve typed, non-destructive failures when P0-01 or P0-02 cannot create an output directory | An existing file at `output_dir` currently escapes the ToolRun interface as `NotADirectoryError` | Add one registry preflight and exact regression coverage; no scientific behavior changes | `implementation_in_progress` |
