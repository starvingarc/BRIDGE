...
DETAILS = {
    ...
    "P0-06": {
        "input": "QC-qualified expression, Cell-State evidence, confirmed developmental context, ProtocolIR metadata, versioned proliferation/stress-response program knowledge, and in method mode an exact caller/data-owner biological-unit attestation receipt.",
    ...
    "P0-08": {
        "input": "A `ToolRequestV2` with empty assets and parameters, one checksummed candidate GateRuleSpec v0.2 bound to ReasonCodeCatalog v0.2, one to five DomainGateInput v0.1 bindings, and their checksummed MeasurementSpecV2, QCReadinessProfileV2, MeasurementResultV2, validation, prior and sensitivity records. A checksummed ProductCase is required whenever a domain declares ProductCase or QC context; its source MeasurementSpec remains independent from each domain MeasurementSpec.",
        ...
        "reject": "Wrong roles, Schema IDs, object versions, logical bindings, checksums, candidate gate bytes, unsafe references, changed inputs or drifted bundles fail with stable reason codes and no scientific result. ProductCase pointers, ProductDefinition, assay, QC-selected sample/preparation and biological-unit manifest must agree with the checksummed ProductCase. Every domain tool must have explicit or generic QC authorization of `eligible` or `conditional`; missing or unknown authorization fails closed. A bound MeasurementResult whose domain MeasurementSpec version disagrees is ineligible. Missing, unknown or unavailable measurement states, or absent paired upstream ToolRun provenance, instead execute as `not_assessed`; negative and alert remain distinct and never become pass/fail.",
