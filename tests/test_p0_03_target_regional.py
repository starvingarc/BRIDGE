...
def _payloads() -> dict[str, dict]:
    return {
        ...
        "measurement_spec": {
            ...
            "version": "1.0.0",
            "scientific_question": "Synthetic target/regional evidence integration",
            "assay": "scRNA-seq",
            "status": "candidate",
            ...
            "raw_metric_definition": {
                "metric": "role-resolved composition"
            },
            "numerator": "configured role count",
            "denominator": "declared composition channel",
            "direction": None,
...
def _bind_upstream_profiles(
    ...
    *,
    ...
    bind_reviewed_biological_units(payloads, view, units=units)
...
def _request(
    ...
    *,
    ...
    _bind_upstream_profiles(values, units=units)
...
def test_shared_composition_helpers_select_group_and_detect_missing_channels() -> None:
    records = [
        UpstreamCompositionRecord(
            view="consensus_supported_only",
            label="state:alpha",
            label_level="L1",
            denominator_view="all observations",
            count=60,
            fraction=0.6,
            denominator=100,
        ),
    ...
    selected = select_composition_records(
        records,
        views=[CompositionView.SOURCE_SPECIFIC],
        label_levels=["L1"],
        source_ids=["reference:one"],
    )
    ...
    assert composition_channel_denominator(selected) == (100, "all observations")
    assert requested_composition_channels_missing(
        grouped,
        views=[CompositionView.SOURCE_SPECIFIC],
        label_levels=["L1"],
        source_ids=["reference:one"],
    ...
    assert requested_composition_channels_missing(
        ...
        views=[CompositionView.SOURCE_SPECIFIC],
        label_levels=["L1"],
        source_ids=["reference:one", "reference:missing"],
    ) is True
    ...
    with pytest.raises(
        ValueError,
        match="cell_state_composition_denominator_mismatch",
    ):
        composition_channel_denominator(inconsistent)


def test_configured_nondefault_roles_drive_denominators_without_code_changes(
    tmp_path: Path,
) -> None:
    payloads = _payloads()
    payloads["target_regional_assessment_spec"].update(
        {
            "regional_denominator_lineage_roles": ["not_target"],
            "whole_product_target_region_roles": ["regional_shift"],
        }
    )
    ...
    assert run.execution_state is ExecutionState.SUCCEEDED
    regional = run.result["regional_fidelity_channels"][0]
    assert regional["target_related_denominator"] == 20
    assert regional["whole_product_target_region_fraction"] == {
        "role": "configured_target_region",
        "numerator": 20,
        "denominator": 100,
        "fraction": 0.2,
    }
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {
            "regional_denominator_lineage_roles": ["unresolved"],
        },
        {
            "whole_product_target_region_roles": ["unresolved"],
        },
    ],
)
def test_assessment_spec_rejects_noop_or_unknown_promoting_configuration(
    tmp_path: Path,
    updates: dict[str, object],
) -> None:
    payloads = _payloads()
    payloads["target_regional_assessment_spec"].update(updates)
...
@pytest.mark.parametrize("state", ["unknown", "ood", "unresolved"])
def test_explicit_upstream_unknown_states_remain_unresolved_and_partial(
    tmp_path: Path,
    state: str,
) -> None:
    payloads = _payloads()
    payloads["cell_state_evidence_profile"]["composition"]["records"][0][
        "state_evidence_state"
    ] = state
    ...
    assert run.execution_state is ExecutionState.PARTIAL
    assert run.result["result_state"] == "partial"
    assert run.result["unmapped_states"][0]["reason_code"] == (
        "upstream_state_unresolved"
    )
    roles = _role_map(run.result["target_identity_channels"][0])
    assert roles["target"]["numerator"] == 0
    assert roles["unresolved"]["numerator"] == 60
    regional = run.result["regional_fidelity_channels"][0]
    assert regional["target_related_denominator"] == 20
    assert regional["whole_product_target_region_fraction"]["numerator"] == 0


def test_upstream_unresolved_label_cannot_be_promoted_by_role_map(
    tmp_path: Path,
) -> None:
    payloads = _payloads()
    payloads["cell_state_evidence_profile"]["unresolved_labels"] = [
        "state:alpha"
    ]
...
def test_explicit_unresolved_role_is_partial_not_complete(tmp_path: Path) -> None:
    payloads = _payloads()
    assignment = payloads["state_role_map"]["assignments"][0]
    assignment["lineage_role"] = "unresolved"
    assignment["regional_role"] = "unresolved"
    ...
    assert "state_role_mapping_incomplete" in run.reason_codes


def test_explicit_unresolved_lineage_cannot_enter_configured_positive_output(
    tmp_path: Path,
) -> None:
    payloads = _payloads()
    assignment = payloads["state_role_map"]["assignments"][0]
    assignment["lineage_role"] = "unresolved"
    assignment["regional_role"] = "regional_shift"
    payloads["target_regional_assessment_spec"][
    ...
    assert run.execution_state is ExecutionState.PARTIAL
    target = _role_map(run.result["target_identity_channels"][0])
    assert target["unresolved"]["numerator"] == 60
    regional = run.result["regional_fidelity_channels"][0]
    assert regional["target_related_denominator"] == 20
    assert regional["whole_product_target_region_fraction"]["numerator"] == 20


@pytest.mark.parametrize(
    "mutate",
    [
        lambda payload: payload["target_identity_channels"][0][
            "role_fractions"
        ][0].update({"numerator": 59, "fraction": 0.59}),
        lambda payload: payload["regional_fidelity_channels"][0].update(
            {"target_related_denominator": 101}
        ),
        lambda payload: payload["regional_fidelity_channels"][0][
            "whole_product_target_region_fraction"
        ].update({"denominator": 99, "fraction": 60 / 99}),
    ],
)
def test_result_model_rejects_cross_field_conservation_failures(
    tmp_path: Path,
    mutate,
) -> None:
    payload = deepcopy(ToolRegistry.load_default().run(_request(tmp_path)).result)
    mutate(payload)

    with pytest.raises(ValidationError):
        TargetRegionalEvidenceResult.model_validate(payload)


def test_single_source_product_case_rejects_mixed_manifest_sources(
    tmp_path: Path,
) -> None:
    run = ToolRegistry.load_default().run(
        _request(
            tmp_path,
            units=[
                ("preparation:demo@1.0.0", "sample:demo@1.0.0"),
                ("preparation:other@1.0.0", "sample:demo@1.0.0"),
            ],
        )
    )

    assert run.execution_state is ExecutionState.FAILED
    assert "product_case_source_unit_binding_mismatch" in run.reason_codes
    assert run.result is None


def test_measurement_spec_must_bind_manifest_unit_kinds(tmp_path: Path) -> None:
    payloads = _payloads()
    payloads["measurement_spec"].update(
        {
    ...
    assert run.execution_state is ExecutionState.FAILED
    assert "measurement_spec_biological_unit_mismatch" in run.reason_codes
    assert run.result is None


def test_assignment_artifact_checksum_and_hierarchy_are_mandatory(
    tmp_path: Path,
) -> None:
    request = _request(tmp_path)
    assignment_ref = next(
        ...
        if ref.role == "biological_unit_assignment"
    )
    payload = json.loads(assignment_ref.path.read_text(encoding="utf-8"))
    payload["assignments"][0]["independence_group_ref"] = (
        "sample:other@1.0.0"
    )
    changed_sha = _write_json(assignment_ref.path, payload)
    changed_ref = assignment_ref.model_copy(update={"sha256": changed_sha})
    request = request.model_copy(
        update={
            "object_inputs": [
                changed_ref if ref is assignment_ref else ref
                for ref in request.object_inputs
            ]
        }
    )
    ...
    assert run.execution_state is ExecutionState.FAILED
    assert "biological_unit_assignment_checksum_mismatch" in run.reason_codes
    assert "biological_unit_assignment_group_mismatch" in run.reason_codes
    assert run.result is None


def test_v1_upstream_profiles_cannot_bypass_the_v2_spine(tmp_path: Path) -> None:
    request = _request(tmp_path)
    changed = []
    for ref in request.object_inputs:
        ...
        elif ref.role == "qc_readiness_profile":
            ref = ref.model_copy(
                update={
        ...
        changed.append(ref)
    request = request.model_copy(update={"object_inputs": changed})
    spec = ToolRegistry.load_default().describe("P0-03")

    eligibility = adapter.check_eligibility(request, spec)

    assert eligibility.eligible is False
    assert "object_input_schema_mismatch" in eligibility.reason_codes
    assert "object_input_version_mismatch" in eligibility.reason_codes


@pytest.mark.parametrize(
    "state",
    ["not_assessed", "unavailable", "unknown", "missing"],
)
def test_non_assessed_upstream_states_remain_distinct(
    tmp_path: Path,
    state: str,
) -> None:
    payloads = _payloads()
    ...
    assert run.result["result_state"] == "not_assessed"
    assert run.result["score_state"] == "unavailable"
    assert run.result["upstream_composition_state"] == state
    assert f"cell_state_composition_{state}" in run.reason_codes


def test_unknown_composition_cannot_carry_assessed_records(tmp_path: Path) -> None:
    payloads = _payloads()
    payloads["cell_state_evidence_profile"]["composition"]["state"] = "unknown"

    run = ToolRegistry.load_default().run(_request(tmp_path, payloads=payloads))

    assert run.execution_state is ExecutionState.FAILED
    assert run.reason_codes == ["cell_state_composition_state_conflict"]
    assert run.result is None


def test_unregistered_composition_state_is_rejected(tmp_path: Path) -> None:
    payloads = _payloads()
    payloads["cell_state_evidence_profile"]["composition"] = {
        "state": "negative",
        "records": [],
    ...
    assert run.execution_state is ExecutionState.FAILED
    assert run.reason_codes == ["cell_state_composition_state_invalid"]
    assert run.result is None


def test_changed_existing_result_bundle_is_refused(tmp_path: Path) -> None:
    first = ToolRegistry.load_default().run(
        _request(
            tmp_path / "first",
            output_dir=tmp_path / "output",
            input_id_prefix="first",
        )
    )
    first.artifacts[0].path.write_text('{"tampered":true}', encoding="utf-8")

    second = ToolRegistry.load_default().run(
        _request(
            tmp_path / "second",
            output_dir=tmp_path / "output",
            input_id_prefix="second",
        )
    )
...
@pytest.mark.parametrize(
    "role,field,value",
    [
        ("qc_readiness_profile", "measurement_spec_version", "2.0.0"),
        ("qc_readiness_profile", "measurement_spec_status", "frozen"),
        ("cell_state_evidence_profile", "measurement_spec_status", "frozen"),
    ],
)
def test_measurement_spec_profile_status_and_version_are_bound(
    tmp_path: Path,
    role: str,
    field: str,
    value: str,
    ...
    payloads = _payloads()
    payloads[role][field] = value

    run = ToolRegistry.load_default().run(_request(tmp_path, payloads=payloads))

    assert run.execution_state is ExecutionState.FAILED
    assert "measurement_spec_profile_binding_mismatch" in run.reason_codes
    assert run.result is None


def test_result_rejects_nonshadow_assessment_and_unpaired_channels(
    tmp_path: Path,
) -> None:
    result = deepcopy(ToolRegistry.load_default().run(_request(tmp_path)).result)

    nonshadow = deepcopy(result)
    nonshadow["upstream_composition_state"] = "unknown"
    with pytest.raises(ValidationError):
        TargetRegionalEvidenceResult.model_validate(nonshadow)

    missing_regional = deepcopy(result)
    missing_regional["regional_fidelity_channels"] = []
    with pytest.raises(ValidationError):
        TargetRegionalEvidenceResult.model_validate(missing_regional)

    mismatched_denominator = deepcopy(result)
    regional = mismatched_denominator["regional_fidelity_channels"][0]
    regional["whole_product_denominator"] = 101
    regional["whole_product_target_region_fraction"].update(
        {"denominator": 101, "fraction": 60 / 101}
    )
    with pytest.raises(ValidationError):
        TargetRegionalEvidenceResult.model_validate(mismatched_denominator)


def test_public_assessment_schema_rejects_source_noop_and_unknown_promotion() -> None:
    payload = _payloads()["target_regional_assessment_spec"]
    validator = Draft202012Validator(
        PUBLIC_SCHEMA_MODELS[
            "bridge://schemas/target-regional-assessment-spec/v0.1"
        ].model_json_schema()
    ...
    source_noop = deepcopy(payload)
    source_noop["source_ids"] = ["reference:one"]
    assert list(validator.iter_errors(source_noop))

    unresolved_denominator = deepcopy(payload)
    unresolved_denominator["regional_denominator_lineage_roles"] = [
        "unresolved"
    ]
    assert list(validator.iter_errors(unresolved_denominator))
