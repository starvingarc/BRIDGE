...
def test_figure_registry_discovers_legacy_and_typed_qc_components() -> None:
    ...
    assert registry.validation_summary() == {
        ...
        "object_version": "0.1.0",
        "component_count": 25,
        "typed_candidate_count": 18,
        "legacy_untyped_count": 7,
        "producer_tool_ids": ["P0-01", "P0-02", "P0-03", "P0-04", "P0-05", "P0-06", "P0-07"],
    }
...
def test_figure_registry_returns_defensive_copies() -> None:
    ...
    assert registry.validation_summary()["component_count"] == 25
    assert len(registry.list(tool_id="P0-01")) == 6
    assert FigureRegistry.load_default().validation_summary()["component_count"] == 25
