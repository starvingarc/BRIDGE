...
from bridge.tool_packages.p0_02_cell_state.reference import load_packaged_marker_programs
...
RUNTIME_METHOD_VERSIONS = {method: "0.4.0" for method in RUNTIME_EXECUTABLE_METHODS}
RUNTIME_TOOL_VERSION = "0.4.0"
RUNTIME_ENVIRONMENT_SPEC = "ENV-CELLSTATE-PY-v0.1"
BENCHMARK_IMPLEMENTATION_VERSION = "0.4.0"
...
def load_pilot_benchmark_spec() -> CellStateBenchmarkSpec:
    return CellStateBenchmarkSpec.model_validate(_load_resource("benchmark_spec_pilot_v0.1.yaml"))
...
def prepare_benchmark_split(
    ...
    *,
    ...
    catalog = yaml.safe_load(asset_catalog_path.read_text(encoding="utf-8"))
...
def validate_probability_output(path: Path) -> set[str]:
    ...
    return {column.removeprefix("prob__") for column in probability_columns}
...
def verify_reviewer_signatures(
    ...
    for item in reviewers:
        ...
        trusted[identity] = item.get("public_key_base64")
    ...
    for signature in signatures:
        ...
        if signature.object_sha256 != expected_hash or not public_key:
...
def validate_release_bundle(
    ...
    _validate_locked_run_bundle(run, split, summary, release, root)
    raw_gate_results = summary.get("gate_results", [])
    gate_results = {
        (str(item.get("metric")), str(item.get("scope"))): item
        for item in raw_gate_results
    }
    gate_passed = all(
        _gate_result_passes(criterion, gate_results.get((criterion.metric, criterion.scope)))
    ...
    state_method_results = summary.get("state_method_results", {})
    for state_id, methods in release.selected_methods.items():
        results = state_method_results.get(state_id, {})
        if any(
            results.get(method, {}).get("state") != "passed"
            or results.get(method, {}).get("implementation_version")
            != release.method_implementation_versions[method]
...
def run_pilot_benchmark(
    ...
    catalog = yaml.safe_load(asset_catalog_path.read_text(encoding="utf-8"))
    assets = {item["asset_id"]: item for item in catalog.get("assets", [])}
...
def _prediction_metadata(path: Path) -> dict[str, Any]:
    ...
    if not metadata_path.is_file():
        native = {
            "source_specific_correlation": "reference_similarity",
            "marker_program_evidence": "marker_program",
        }
        family = native.get(path.stem)
        if family is None:
            raise BenchmarkError("adapter_metadata_not_found", path.stem)
        return {
            "adapter": "native",
            "adapter_implementation_version": BENCHMARK_IMPLEMENTATION_VERSION,
    ...
    metadata.setdefault("independent_evidence_vote", True)
    return metadata
...
def _pareto_candidates(metrics: dict[str, dict[str, Any]]) -> list[str]:
    ...
    for method, result in metrics.items():
        if (
            not result.get("independent_evidence_vote", True)
            or result.get("evaluation_protocol", "inductive") != "inductive"
        ):
            continue
        dominated = any(
            other != method
            and other_result.get("independent_evidence_vote", True)
            and other_result.get("evaluation_protocol", "inductive") == "inductive"
            ...
            for other, other_result in metrics.items()
        )
        if not dominated:
