...
RESULT_SCHEMA_REF = "bridge://schemas/off-target-control-result/v0.1"
...
class OffTargetControlAdapter:
    ...
    def run(self, request: ToolRequestV2, spec: ToolPackageSpecV2) -> ToolRunV2:
        ...
        try:
            result = evaluate_off_target_control(
                run_id=run_id,
            ...
            if not reason.startswith("cell_state_composition_"):
                reason = "off_target_evaluation_failed"
        ...
        try:
            output_file = publish_single_json(
                request=request,
                run_id=run_id,
                filename="off_target_control_result.json",
                payload=payload,
            )
        ...
        artifact = ArtifactManifest(
            artifact_id=f"artifact:{run_id}:off-target-result",
            kind="off_target_control_result",
            path=output_file,
...
def _envelope_reasons(
    ...
    if request.measurement_spec_ref is not None:
        reasons.append("p0_05_measurement_spec_parameter_forbidden")
    if request.parameters:
        reasons.append("p0_05_parameters_forbidden")
    if request.random_seed != 0:
        reasons.append("p0_05_random_seed_forbidden")
...
def _binding_reasons(
    ...
    if qc_profile.readiness_state in {
        ...
        reasons.append("qc_not_ready_for_off_target_evidence")
    try:
