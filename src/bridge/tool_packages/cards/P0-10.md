...
The research statement registry selects the separately versioned research result.
An optional `research_report_context` input binds confirmed product facts, the
exact DataView, candidate-development limitations, composition labels, observed
mean counts and unverified explanation history to snapshot v0.3. The same
snapshot presentation is consumed by the results page and offline attachments;
v0.2 snapshots and historical artifact bytes remain readable and unchanged.
Exact graph, case, measurement, input-revision and statement hashes are checked
before private Chinese HTML and JSON/CSV/SVG artifacts are emitted. This branch
does not authorize qualified biological conclusions or public release.
