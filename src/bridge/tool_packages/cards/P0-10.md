...
The separately versioned research AnalysisSnapshot and statement registry are
accepted through the registered research snapshot role. Exact graph, case,
measurement, input-revision and statement hashes are checked before private
Chinese offline HTML and JSON/CSV/SVG artifacts are emitted. This branch uses a
research verification result, not the legacy qualified/public release result.
Historical report artifacts remain immutable across later input corrections.
