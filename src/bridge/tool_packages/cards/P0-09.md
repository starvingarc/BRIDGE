...
- `external_case_evidence_refs` has set semantics: an exactly identical same-source declaration is idempotently collapsed before identity and graph construction. The same versioned Evidence ref with different content, Claim, profile, family, relation, lifecycle, or source graph is not collapsed; every conflicting declaration is sanitized as `duplicate_logical_key_conflict`.
...
Top-level role, Schema, media type, checksum, graph scope, prior-chain, overlapping/unusable output path, legacy-score, unsafe publication reference, or input-mutation failures return `failed`, `result=null`, and no artifacts. Output-root or staging creation failure is typed and never overwrites or removes a pre-existing output-path file. A legal absolute output path may contain `=` in a parent basename; write-after-read Parquet validation parses stable file bytes rather than invoking Hive partition discovery on that path. Public `candidate_records`, `missing_observations`, and `external_case_evidence_refs` are strict object arrays: a non-object, extra field, or other registered-Schema violation fails the public registry path. The module-direct adapter seam can preserve an object-shaped invalid sibling long enough to emit sanitized `partial` metadata, but this does not relax the public Schema; rejected payloads do not enter the graph or get echoed. A Schema-valid candidate/missing item or Comparison external mapping that fails record-level provenance/binding semantics is likewise sanitized into `partial` metadata only. The bounded publication contract rejects local/server paths, `file:` URIs, home-variable paths, credential-like assignments/tokens, and non-identifier public refs; it is not a promise of general secret detection. Contract-complete absence is a scientific `EvidenceRequirement`/`insufficient_evidence`, not a technical failure or zero.
...
Each `StructuredInputRef` requires `input_id:string`, exact `role:string`, registered `schema_ref:string`, payload `object_version:string`, absolute `path`, lowercase 64-hex `sha256`, and `media_type="application/json"`. Exact bytes are checked before parse and immediately before publication. `input_id` is a request-local binding label: consistent renaming of it and every bundle binding does not alter the semantic input hash, run ID or reusable bundle bytes. The reusable bundle stores role/Schema/version/media/semantic-checksum identities plus a deterministic occurrence index, never caller labels; raw checksums remain in the ToolRun request and content-addressed graph inputs additionally retain their source-byte checksum.
...
Case manifest adds `product_case_ref`; Comparison manifest adds `comparison_ref`, sorted pure `CaseGraphRef` objects and sorted public `external_evidence_bindings`. Both contain graph/version, canonicalization ID, node/edge/object counts, source semantic input hash, optional pure base graph ref, fixed allowlisted basenames, exact checksums for three JSON fact sets and two Parquet tables, exact Parquet row counts, and deterministic UTC time. The three fact-set IDs are bound to the source input hash; graph IDs are deterministically derived from the versioned root. `object_counts` contains only positive entries and must equal the exact `GraphNodeType` counter, not merely the total. Query open parses the exact already-checksummed Parquet bytes, then rejects absolute/traversal names, symlinks, checksum drift, row/count/graph drift, disconnected projections, JSON-fact-to-Parquet disagreement, non-canonical properties, forged identities/self-hashes, or any edge set that differs from the canonical facts. Owned record tier/lifecycle and deterministic Evidence/Requirement/Reconciliation identities are re-derived. A satisfied requirement may cite only the latest active, formal, applicable evidence for the same Claim/ProductCase and a reviewed family of the required channel. Before compilation, every supplied base/source manifest is opened against its real directory and all five authoritative artifacts. Comparison external nodes must uniquely match an actual source EvidenceRecord and the stable public binding, including the exact versioned P0-08 profile; an evidence-to-profile `depends_on` edge makes the profile explicit, while the unique evidence-to-source-Case edge carries the canonical SHA-256 of the complete stable binding. Query reconstruction therefore binds source Claim, evidence state, applicability, ToolRun state and every other binding field to authoritative Parquet facts without copying source scientific properties. This digest is an internal manifest-to-Parquet consistency binding, not an independent trust anchor: `open` does not verify a signature or authenticity and cannot detect a coordinated manifest/Parquet/checksum rewrite when the source Case artifacts are absent. Source scientific facts are checked during compilation against checksummed source Case manifests and record sets; later external-provenance queries stop with `source_case_graph_required`. The public `open` boundary covers manifest JSON/models, artifact reads/checksums, Parquet rows/models, JSON fact models and both Case/Comparison projections; every nested failure becomes exact `manifest_integrity_failed` with no exception cause and no attacker-controlled payload or artifact path in its rendered traceback. External nodes retain `properties_json=null` and stop provenance traversal at the case boundary.
