...
| Result schema | `bridge://schemas/evidence-compiler-result/v0.2` |
...
Package 0.5.1 retains historical sufficiency profiles referenced by immutable prior records from the verified canonical base graph when a case append supplies a newer P0-08 run. Historical profiles cannot supply current candidates or missingness observations; identity/content collisions are refused.

## Registered read-only queries

Package 0.5.0 adds `case_query` and `comparison_query`. Each accepts exactly one
`evidence_graph_manifest` with its matching existing Case/Comparison Schema and
payload graph version, and one `evidence_graph_query` with
`bridge://schemas/evidence-graph-query/v0.1`, object version `0.1.0`.
Compilation roles cannot mix with query roles. Both query modes require
`assets=[]`, `parameters={}`, and `measurement_spec_ref=null`.

The query object has top-level `object_version`, `query_name` and only the named
arguments of the selected method in the seven-query table below. Unknown fields,
wrong selectors and non-strict numeric/boolean types fail closed. Claim-evidence
and Case-subgraph queries require explicit `evidence_tiers`; use
`["formal", "shadow", "exploratory"]` when investigating candidate gaps.
Family and conflict queries retain the existing cross-tier behavior.
Lifecycle filters retain existing method semantics; an inactive record has no
current support/contradiction edge, so use family/provenance queries to audit it.

The result union `bridge://schemas/evidence-compiler-result/v0.2` accepts the
unchanged compilation result or the unchanged `EvidenceGraphQueryResult`.
The old compilation-result Schema stays registered. A query ToolRun carries
the exact source graph ID/version and a path-free content hash bound to the
manifest checksum and normalized query. It has no measurements, visualizations
or artifacts and does not create an output directory; the ToolRun itself is its
query receipt. Query run IDs are namespaced separately from compilation IDs.
Equivalent requests differing only in request ID/output location return the
same facts and content identity.

All five authoritative graph sidecars are verified using existing graph-open
validation and checked for changes before returning. Manifest/sidecar tampering,
symlinked paths, output/source overlap, wrong graph kind and mixed roles fail
without publishing results. Query payloads contain no local paths; the private
ToolRun request retains normal local input references. Comparison external
provenance still stops with `source_case_graph_required`; queries do not add
source facts or alter graph lineage.

## Inputs

The following roles apply to compilation modes only.
...
| `evidence_compiler_visualization_data.json` | Typed source for the three explanatory figures |
| three `.tsv` tables | Complete claim, family-relation and requirement/exclusion records |
| nine `.svg/.png/.pdf` renders | Three deterministic static figures in three formats |
| `evidence_compiler_visualization_artifact_set.json` | Figure contracts and exact data/table/render bindings |
| `artifact_manifest.json` | Checksums and sizes of the other 23 files; no circular self-hash |

The three figures explain each available reconciliation, dependency-aware family relations within those reconciliations, and current requirements/exclusions. A registry Claim without evidence or an explicit requirement produces no row; that absence is neither a zero nor a missing-evidence observation. Static row-capacity overflow keeps the complete TSV and returns a table-fallback figure; it never applies top-N selection. Open requirements, claim-level reconciliation exclusions and exact rejected inputs remain separate. Record and family counts are audit counts, not independent evidence, and a missing requirement is not a measured zero.

`measurements=[]` and `visualizations=[]`; portable figure contracts are carried by the package-owned artifact set because the shared ToolRun envelope is unchanged. P0-09 computes or populates no `domain_score`, total score, grade, pass/fail, potency, safety, efficacy, GMP-release or ranking field. A bound P0-08 profile's required `domain_score=null` is preserved only as upstream provenance and never becomes a P0-09 conclusion. JSON and Parquet are the graph facts; NetworkX validates and serves bounded in-process queries only. LadybugDB is deferred shadow work and is not required.
...
- P0-08 v0.1 profiles bind ProductCase, MeasurementSpec and MeasurementResult by bare ID for compatibility. P0-08 v0.2 profiles bind all three by exact `object_id` plus `object_version`. Both are accepted for shadow/exploratory compilation. Neither profile version proves the version of each retained EvidenceFamily because `deduplicated_evidence_family_ids` remains unversioned, so every formal candidate/external ref is still conservatively rejected with `sufficiency_profile_version_binding_unavailable`.
...
Selected catalog methods are the internal deterministic engine, bounded read-only API, PyArrow/Parquet storage, and NetworkX graph validation. They remain `formal_eligible=false`; catalog formal-eligible count remains zero. Dependency licenses and exact environment pins, including Matplotlib for deterministic static figures, are governed by the packaged knowledge snapshot and `ENV-EVIDENCE-v0.2`.
...
| `tool_version` | literal `0.5.1` or null | no | If present, must match exactly |
...
`artifact_manifest.json` records run/tool/environment/result schema, caller-label-free semantic identities of structured objects, and exact checksums/media types/available byte sizes of the other 23 files. It deliberately does not hash itself. Raw input SHA remains in the current `ToolRunV2.request`; base/source graph inputs also retain their content-addressed source SHA in the reusable manifest. Semantically equivalent set-order or consistent input-label variants reuse the same byte-identical bundle. `ToolRunV2.artifacts` adds absolute deployment paths for local retrieval; those paths never enter scientific JSON/Parquet facts.

Every `EvidenceGraphQueryResult` contains query name, graph ID/version, sorted node/edge objects, returned and omitted counts, `truncated`, and stable reason codes. `truncated=true` only when at least one reachable node or edge was actually omitted. It never exposes arbitrary predicates, executable query strings or mutation methods. The generated query Schema enforces the same selector XOR, claim-version dependency and unique domain/tier filters as runtime validation; optional null selectors retain their runtime meaning.
