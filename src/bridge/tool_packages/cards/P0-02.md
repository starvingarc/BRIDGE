...
artifact manifest remain supported. When the deployment-owned
`BRIDGE_QC_PROFILE_CATALOG` record also provides `v2_path` and
`v2_sha256`, P0-02 verifies that QC V2 object and its exact selected
DataView, then adds a checksummed `cell_state_evidence_profile_v3.json`
artifact to the ToolRun.

The V3 sidecar binds assay, the canonical MeasurementSpec SHA, annotation
vocabulary, reference manifest, QC V2 bytes, selected-view bytes and
observation IDs, producer version, and environment. Its L1 composition rows
...
content-addressed. Missing catalog V2 fields leave the v0.1 run successful but
report the V3 handoff as unavailable; no binding is inferred. Any replacement
of the checksummed QC v1 or V2 bytes during execution fails closed.
