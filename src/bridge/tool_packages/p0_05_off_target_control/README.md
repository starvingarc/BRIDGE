...
- **Input:** six checksummed JSON objects for compatible aggregation, or nine for
  ...
  may add one authorized checksummed `MeasurementSpecV2`; projected runs also require the reviewed biological-unit manifest.
- **Output:** `OffTargetControlProfile`; method mode adds an
  ...
  detectability and supplied OOD channel states. P0-05 v0.5.1 always returns profile
...
Projection uses an independent P0-05 MeasurementSpec and verifies its assay, product applicability, analysis unit, independence group and cell/nucleus observation unit against the reviewed manifest. Projection keeps the original domain records in profile v0.2. Missing,
