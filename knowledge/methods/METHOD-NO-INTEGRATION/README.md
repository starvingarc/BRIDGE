# No integration

| Field | Value |
|---|---|
| Method ID | `METHOD-NO-INTEGRATION` |
| Modules | P0-02 |
| Scientific status | shortlisted |
| Source status | `internal_no_public_source` |
| License status | `internal` |
| Version | `unresolved` (`not_frozen`) |
| Maintenance | `requires_live_review` |
| Primary paper | `not_applicable` |
| Evidence family | EF-CS-NO-INTEGRATION |
| Retrieval policy | `registered_local_snapshot` |

## BRIDGE Use

cell-state evidence

## Inputs

normalized expression/PCA by source | all eligible annotation targets

## Outputs

uncorrected embedding and annotation outputs | unintegrated annotation and composition

## Boundaries

Marker/program evidence always retains a non-integrated view. | Never omit because an integrated view looks cleaner

## Environment

Python 单细胞核心环境

## Curation Notes

No method-specific correction is recorded in this snapshot.

## Official Sources

- No public source registered; see `source_status`.

Source verification records confirm provenance and accessibility only; they do not promote scientific status.
